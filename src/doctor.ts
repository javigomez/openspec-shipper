...
import { readOrchesterConfig, type OrchesterConfig } from "./orchester-config";
...
export async function runDoctor(projectDir: string): Promise<DoctorCheck[]> {
  const config = (await readOrchesterConfig(projectDir)) ?? undefined;
  ...
  checks.push(checkCommand("opencode", ["--version"], projectDir, "OpenCode CLI is available"));
  ...
  checks.push(
    config
      ? ok("orchester config", ".orchester/config.json found")
      : warning("orchester config", ".orchester/config.json missing; run `bun run target:init <repo>`"),
  );
    ...
    for (const script of REQUIRED_PACKAGE_SCRIPTS) {
      checks.push(
        packageJson.scripts?.[script]
          ...
          : warning(`script:${script}`, `package script ${script} missing; configure .orchester/config.json if your repo uses another command`),
      );
    }
  }
...
function packageManagerCommand(config: OrchesterConfig | undefined): string {
