...
export function defaultConfig(): RunnerConfig {
  ...
  const stateDir = join(rootDir, ".orchester");

  return {
    rootDir,
    projectDir: requiredEnv("PROJECT_DIR"),
    queuePath: process.env.QUEUE_PATH ?? join(rootDir, "queue.md"),
    stateDir,
    opencodeBin: requiredEnv("OPENCODE_BIN"),
    opencodeModel: optionalEnv("OPENCODE_MODEL"),
    opencodePrintLogs: process.env.OPENCODE_PRINT_LOGS === "1",
    opencodeLogLevel: optionalEnv("OPENCODE_LOG_LEVEL"),
    opencodeStats: process.env.OPENCODE_STATS === "1",
    opencodeStatsIntervalMs: parsePositiveInt(process.env.OPENCODE_STATS_INTERVAL_MS, DEFAULT_STATS_INTERVAL_MS),
    opencodeStatsTimeoutMs: parsePositiveInt(process.env.OPENCODE_STATS_TIMEOUT_MS, DEFAULT_STATS_TIMEOUT_MS),
    opencodeStatsProject: process.env.OPENCODE_STATS_PROJECT ?? "",
    opencodeStatsModels: optionalEnv("OPENCODE_STATS_MODELS"),
    opencodeStatsDays: optionalEnv("OPENCODE_STATS_DAYS"),
    loopDelayMs: parsePositiveInt(process.env.ORCHESTER_LOOP_DELAY_MS, DEFAULT_LOOP_DELAY_MS),
...
export async function runQueue(mode: RunnerMode, config: RunnerConfig): Promise<number> {
  ...
  const args = buildConfiguredOpenCodeArgs(config, pendingTask);

  if (mode === "dry-run") {
    console.log(`Next task: ${pendingTask.rawCommand}`);
    console.log(`Command: ${formatCommand(config.opencodeBin, args)}`);
  ...
  return await runSingleTaskWithLock(config, queue.lines, pendingTask, args);
}
...
async function runSingleTaskWithLock(
  ...
  try {
    ...
    return await executeTask(config, lines, task, args);
  } finally {
    await lock.release();
  }
}
...
async function executeTask(
  ...
  console.log(`Command: ${formatCommand(config.opencodeBin, args)}`);
  console.log(`Cwd: ${config.projectDir}`);
  console.log(`Env PWD: ${config.projectDir}`);
  console.log(`Log: ${relative(config.rootDir, logPath)}`);

  const result = await executor(config.opencodeBin, args, {
  ...
  const failureSignal = detectFailureSignal(result.output);
...
function buildConfiguredOpenCodeArgs(config: RunnerConfig, task: QueueTask): string[] {
  const taskArgs = buildOpenCodeArgs(task);
  const args = ["run"];

  if (config.opencodePrintLogs) {
    args.push("--print-logs");
  }

  if (config.opencodeLogLevel) {
    args.push("--log-level", config.opencodeLogLevel);
  }
